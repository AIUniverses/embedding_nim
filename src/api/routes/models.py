...
async def get_model_info(model_id: str, request: Request):
    ...
    try:
        ...
        logger.error(f"Error getting model info for {model_id}: {str(e)}", exc_info=True)
        raise HTTPException(
...
async def load_model(model_id: str, request: Request):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to load model {model_id}: {reason}"
            )
...
async def get_model_status(model_id: str, request: Request):
    ...
    try:
        ...
        logger.error(f"Error getting model status for {model_id}: {str(e)}", exc_info=True)
        raise HTTPException(
